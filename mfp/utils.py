...
def calc(src: Source, env: Troposphere, range_bounds_m, measures: List[Measure], search_area :SearchArea):
    ...
    show(fwd_field, bartlett_mfp_field, mv_mfp_field, src, measures)
...
def show(fwd_field, bartlett_mfp_field, mv_mfp_field, src: Source, measures: List[Measure]):
    ...
    extent_fwd = [fwd_field.x_grid[0] * 1E-3, fwd_field.x_grid[-1] * 1E-3, fwd_field.z_grid[0], fwd_field.z_grid[-1]]
    ...
    for m in measures:
        ax[0].plot(m.x_m*1e-3, m.height_m, '*', color='black')

    field = 10 * np.log10(np.abs(bartlett_mfp_field.field.T[::-1, :]))
    f_min, f_max = np.min(field), np.max(field)
    norm_bartlett = Normalize(f_max - 5, f_max)
    extent_bartlett = [bartlett_mfp_field.x_grid[0] * 1E-3, bartlett_mfp_field.x_grid[-1] * 1E-3,
    ...
    extent_mv = [mv_mfp_field.x_grid[0] * 1E-3, mv_mfp_field.x_grid[-1] * 1E-3, mv_mfp_field.z_grid[0],
