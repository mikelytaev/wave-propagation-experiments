...
def calc(src: Source, true_env: Troposphere, range_bounds_m, measures: List[Measure], search_area: SearchArea,
    ...
    kdc = KnifeEdgeDiffractionCalculator(src=src, env=true_env, min_range_m=range_bounds_m[0],
    ...
    fields = fields_from_measures(measures=measures, env=expected_env, search_area=search_area)
    bartlett_mfp_field = rwp_mfp(measures, fields, bartlett_mfp)
    mv_mfp_field = rwp_mfp(measures, fields, mv_mfp)

    show(fwd_field, bartlett_mfp_field, mv_mfp_field, src, measures, true_env)
