...
from experimental.rwp_jax import RWPGaussSourceModel, RWPComputationalParams, \
    TroposphereModel
import jax.numpy as jnp

from experiments.optimization.node.flax.utils import MLPNProfileModel
from experiments.optimization.node.npe.common import adam, RWPModel, add_noise, evaporation_duct_N
...
model = RWPModel(
    ...
    measure_points_z=round(jnp.linspace(5, 200, 25)),
)

measure_evaporation_duct = model.apply_profile(evaporation_duct_N)
measure_evaporation_duct = add_noise(measure_evaporation_duct, 30)

layers = [50]*4
evaporation_duct_opt_res = adam(
    model,
    measure_evaporation_duct,
    ground_truth_profile=evaporation_duct_N,
    learning_rate=0.05,
    gamma=1e-3,
    profile_model=MLPNProfileModel(z_max_m=100.0, layers=layers)
)

model.apply_profile(evaporation_duct_opt_res.res_profile)
...
#     env=env,
# )
# f1 = vis_model.calc_field(evaporation_duct_N)
...
# ax[0].set_ylim([0, 250])
#
# f1 = vis_model.calc_field(evaporation_duct_opt_res.res_profile)
