...
from experiments.optimization.node.npe.common import adam, RWPModel, add_noise, surface_based_duct3_N
...
measure_evaporation_duct = model.apply_profile(surface_based_duct3_N)
measure_evaporation_duct = add_noise(measure_evaporation_duct, 30)

layers = [50]*4
surface_based_opt_res = adam(
    model,
    measure_evaporation_duct,
    ground_truth_profile=surface_based_duct3_N,
    learning_rate=0.05,
    gamma=1e-3,
    profile_model=MLPNProfileModel(z_max_m=100.0, layers=layers)
)

model.apply_profile(surface_based_opt_res.res_profile)
...
#     env=env,
# )
# f1 = vis_model.calc_field(surface_based_duct3_N)
...
# ax[0].set_ylim([0, 250])
#
# f1 = vis_model.calc_field(surface_based_opt_res.res_profile)
