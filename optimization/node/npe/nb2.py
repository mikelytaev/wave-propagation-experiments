...
measure = model.apply_profile(p2)
...
layers = [50]*4
opt_res = adam(model, measure, profile_model=MLPNProfileModel(layers=layers))

z_grid_o = jnp.linspace(0, 250, 250)
model.apply_profile(p2)
plt.plot(model.env.M_profile(z_grid_o), z_grid_o)
model.apply_profile(opt_res.res_profile)
#model.env.N_profile.params = opt_params_0
plt.plot(model.env.M_profile(z_grid_o), z_grid_o)
model.apply_profile(opt_res.start_profile)
...
vis_model = RWPModel(params=RWPComputationalParams(
        ...
        dx_m=100,
        dz_m=1),
    src=RWPGaussSourceModel(freq_hz=3E9, height_m=25.0, beam_width_deg=3.0)
)
