...
from experiments.optimization.node.npe.common import adam, RWPModel, add_noise, PLNPM
...
best_params, opt_params_0 = adam(model, measure, profile_model=PLNPM(z_grid_m=jnp.linspace(0, 100, 50), N_vals=jnp.linspace(0, 1, 50)))

z_grid_o = jnp.linspace(0, 250, 250)
model.apply_N_profile(p2)
plt.plot(model.env.M_profile(z_grid_o), z_grid_o)
model.apply_N_profile(PLNPM(z_grid_m=jnp.linspace(0, 100, 50), N_vals=jnp.zeros(50)))
