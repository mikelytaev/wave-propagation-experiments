...
im = ax[0].imshow(20*jnp.log10(abs(f1+1e-16)).T[::-1,:], extent=extent, aspect='auto', norm=norm, cmap=plt.get_cmap('jet'))
...
#norm = Normalize(-70, -10)
#extent = (vis_model.fwd_model.x_output_grid()[0], vis_model.fwd_model.x_output_grid()[-1]*1e-3, vis_model.fwd_model.z_output_grid()[0], vis_model.fwd_model.z_output_grid()[-1])
im = ax[1].imshow(20*jnp.log10(abs(f2+1e-16)).T[::-1,:], extent=extent, aspect='auto', norm=norm, cmap=plt.get_cmap('jet'))
ax[1].set_xlabel("Range (km)")
ax[1].set_yticklabels([])
ax[1].grid(True)
ax[1].set_ylim([0, 250])
f.colorbar(im, ax=ax, shrink=0.4, location='bottom')
#f.tight_layout()
plt.show()
...
f, ax = plt.subplots(1, 2, figsize=(6, 3.2), constrained_layout=True)
ax[0].plot(surface_duct_opt_res.loss_vals, color='blue', label="Surface duct")
ax[0].plot(elevated_duct_opt_res.loss_vals, color='red', label="Elevated duct")
ax[0].plot(surface_based_duct2_opt_res.loss_vals, color='green', label="Surface based duct")
ax[0].plot(surface_based_duct_opt_res.loss_vals, color='gray', label="Mixed duct")
ax[0].set_xlim([0, 250])
...
ax[1].plot(surface_duct_opt_res.ground_truth_errors, color='blue', label="Surface duct")
ax[1].plot(elevated_duct_opt_res.ground_truth_errors, color='red', label="Elevated duct")
ax[1].plot(surface_based_duct2_opt_res.ground_truth_errors, color='green', label="Surface based duct")
ax[1].plot(surface_based_duct_opt_res.ground_truth_errors, color='gray', label="Mixed duct")
ax[1].set_xlim([0, 250])
...
legend_elements = [Line2D([0], [0], color='blue', lw=1, label='Surface duct'),
                   Line2D([0], [0], color='red', lw=1, label='Elevated duct'),
                   Line2D([0], [0], color='green', lw=1, label='Surface based duct'),
                   Line2D([0], [0], color='gray', lw=1, label='Mixed duct')]
f.legend(handles=legend_elements, loc='lower center', bbox_to_anchor=(0.5, -0.00), ncol=2)
f.tight_layout()
